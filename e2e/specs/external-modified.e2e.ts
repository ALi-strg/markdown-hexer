...
describe("ALi-md-editor Externally-Modified detection", () => {
  const externalPath = path.join(
    os.tmpdir(),
    `alimd-e2e-external-${Date.now()}.md`,
  );
  ...
  async function stubExternalChoice(choice: string) {
    await browser.execute((c) => {
      localStorage.setItem("alimd:e2e:external-choice", c);
    }, choice);
  }
  ...
  it("silently reloads a clean Document whose file changed on disk", async () => {
    ...
    await browser.waitUntil(
      async () =>
        (await browser.getTitle()) === `${externalFilename} — ALi-md-editor`,
